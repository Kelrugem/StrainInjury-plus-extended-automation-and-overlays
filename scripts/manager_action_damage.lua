...
function modDamage(rSource, rTarget, rRoll)
	...
	-- Handle Injury
    local bInjury = ModifierStack.getModifierKey("DMG_INJURY");
    if bInjury then
        table.insert(aAddDesc, "[INJURY]");
    end
...
function getDamageAdjust(rSource, rTarget, nDamage, rDamageOutput, bImmune, bFortif, tags)
	...
	-- RESULTS
	-- KEL add injury
	return nDamageAdjust, nNonlethal, bVulnerable, bResist, aInjury;
	-- END
end
...
function applyDamage(rSource, rTarget, bSecret, sRollType, sDamage, nTotal, bImmune, bFortif, tags)
	...
	if rDamageOutput.sType == "heal" or rDamageOutput.sType == "fheal" then
		-- CHECK COST
		if nInjury <= 0 and nWounds <= 0 then
	...
	-- Damage
		...
		local nDamageAdjust, nNonlethalDmgAmount, bVulnerable, bResist, aInjury = ActionDamage.getDamageAdjust(rSource, rTarget, nTotal, rDamageOutput, bImmune, bFortif, tags);
		if aInjury then
